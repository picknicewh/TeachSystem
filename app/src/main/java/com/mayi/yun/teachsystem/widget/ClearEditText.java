...
public class ClearEditText extends AppCompatEditText {
    ...
    //按钮资源
    private final int CLEAR = R.mipmap.ic_clear;
    //动画时长
    ...
    public void initData(String initLab){
        this.initLab = initLab;
        this.setText(initLab);
    }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        if (text.toString().contains("\n")){
            String mText = getText().toString().replaceAll("\\n","");
            setText(mText);
        }
        if(text.length()>0) {
            if (text.toString().equals(initLab)){
                if (isVisible) {
                    isVisible = false;
                    startGoneAnimator();
                }
